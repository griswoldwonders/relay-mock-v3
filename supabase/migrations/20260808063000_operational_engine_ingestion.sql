-- Relay Rider operational engine: privacy-minimized CSV ingestion with provenance.
...
begin
  ...
  for item,row_no in select value,ordinality from jsonb_array_elements(rows) with ordinality loop
    ...
    if cardinality(errors)=0 then
      ...
      values(org_id,new_run,row_no::integer,'roster',item,jsonb_build_object('display_ref',masked_ref,'site_id',default_site_id,'cohort_id',cohort_id),true,'{}'::text[]);
      ...
      insert into public.ingestion_rows(organization_id,ingestion_run_id,row_number,record_type,raw_record,normalized_record,is_valid,validation_errors)
      values(org_id,new_run,row_no::integer,'roster',item,'{}'::jsonb,false,errors);
  ...
  if source_id is not null then
    ...
    set status=case when invalid_count=0 then 'healthy' else 'partial' end,
        last_synced_at=now(),
        coverage_summary=format('%s roster rows accepted; %s rows require review',valid_count,invalid_count)
    where id=source_id and organization_id=org_id;
  end if;
...
$$;
revoke all on function public.import_cohort_roster(uuid,uuid,uuid,text,text,jsonb,uuid) from public, anon;
grant execute on function public.import_cohort_roster(uuid,uuid,uuid,text,text,jsonb,uuid) to authenticated;
...
begin
  ...
  for item,row_no in select value,ordinality from jsonb_array_elements(rows) with ordinality loop
    ...
    if jsonb_typeof(item->'travel_days')='array' then
      ...
      begin
        select coalesce(array_agg(v::smallint order by v::smallint),'{}'::smallint[]) into days
        from jsonb_array_elements_text(item->'travel_days') v
        where v ~ '^[0-6]$';
      ...
      errors:=array_append(errors,'travel_days must be an array');
    end if;

    if coalesce(item->>'flexibility_minutes','')<>'' then
      if (item->>'flexibility_minutes') ~ '^[0-9]{1,3}$' then
        flex_value := (item->>'flexibility_minutes')::integer;
        if flex_value>240 then errors:=array_append(errors,'flexibility_minutes must be between 0 and 240'); end if;
    ...
    if coalesce(item->>'parking_difficulty','')<>'' then
      if (item->>'parking_difficulty') ~ '^[1-5]$' then parking_value:=(item->>'parking_difficulty')::smallint;
      else errors:=array_append(errors,'parking_difficulty must be 1 through 5'); end if;
    end if;

    case lower(coalesce(item->>'access_point_willing','false'))
      when 'true' then access_value:=true; when '1' then access_value:=true; when 'yes' then access_value:=true;
      when 'false' then access_value:=false; when '0' then access_value:=false; when 'no' then access_value:=false;
      else errors:=array_append(errors,'access_point_willing must be true or false');
    end case;
    ...
    exception when others then
      errors:=array_append(errors,'time fields must use HH:MM format');
    end;
    ...
    if cardinality(errors)=0 then
      ...
      insert into public.ingestion_rows(organization_id,ingestion_run_id,row_number,record_type,raw_record,normalized_record,is_valid,validation_errors)
      values(org_id,new_run,row_no::integer,'commute',item,jsonb_build_object(
        'display_ref',masked_ref,'origin_zone',origin_value,'destination_zone',destination_value,'travel_days',to_jsonb(days),
        'arrival_start',arrival_start_value,'arrival_end',arrival_end_value,'return_start',return_start_value,'return_end',return_end_value,
        'flexibility_minutes',flex_value,'current_mode',nullif(trim(item->>'current_mode'),''),'parking_difficulty',parking_value,
        'access_point_willing',access_value,'ev_hybrid_status',ev_value,'site_id',default_site_id,'cohort_id',default_cohort_id,'approximate_zones',true
      ),true,'{}'::text[]) returning id into source_row;

      insert into public.participant_roster_entries(organization_id,site_id,external_ref_hash,display_ref,status,source_run_id)
      values(org_id,default_site_id,ref_hash,masked_ref,'active',new_run)
      on conflict (organization_id,external_ref_hash) do update
        set site_id=coalesce(excluded.site_id,public.participant_roster_entries.site_id),display_ref=excluded.display_ref,
            status=case when public.participant_roster_entries.status='withdrawn' then 'withdrawn' else 'active' end,
      ...
      if default_cohort_id is not null and (select status from public.participant_roster_entries where id=roster_id)<>'withdrawn' then
        insert into public.roster_cohort_memberships(organization_id,roster_entry_id,cohort_id,source_run_id,status)
        values(org_id,roster_id,default_cohort_id,new_run,'active')
        on conflict (roster_entry_id,cohort_id) do update set status='active',source_run_id=excluded.source_run_id;
      end if;

      if (select status from public.participant_roster_entries where id=roster_id)<>'withdrawn' then
        insert into public.commute_observations(
          organization_id,site_id,cohort_id,roster_entry_id,source_run_id,source_row_id,origin_zone,destination_zone,travel_days,
          arrival_start,arrival_end,return_start,return_end,flexibility_minutes,current_mode,parking_difficulty,access_point_willing,ev_hybrid_status,approximate_zones,status
        ) values(
          org_id,default_site_id,default_cohort_id,roster_id,new_run,source_row,origin_value,destination_value,days,
          arrival_start_value,arrival_end_value,return_start_value,return_end_value,flex_value,nullif(trim(item->>'current_mode'),''),parking_value,access_value,ev_value,true,'active'
        );
      end if;
      valid_count:=valid_count+1;
    else
      insert into public.ingestion_rows(organization_id,ingestion_run_id,row_number,record_type,raw_record,normalized_record,is_valid,validation_errors)
      values(org_id,new_run,row_no::integer,'commute',item,'{}'::jsonb,false,errors);
      invalid_count:=invalid_count+1;
    end if;
  end loop;
  ...
  if source_id is not null then
    update public.data_sources
    set status=case when invalid_count=0 then 'healthy' else 'partial' end,last_synced_at=now(),
        coverage_summary=format('%s commute rows accepted; %s rows require review',valid_count,invalid_count)
    where id=source_id and organization_id=org_id;
  end if;

  if invalid_count>0 then
    insert into public.operational_tasks(organization_id,category,priority,title,detail,subject_type,subject_id,created_by)
    values(org_id,'ingestion_validation','high','Review commute CSV validation',format('%s of %s commute rows require correction or exclusion.',invalid_count,total_count),'ingestion_run',new_run::text,uid);
  end if;

  run_id:=new_run; row_count:=total_count; valid_row_count:=valid_count; invalid_row_count:=invalid_count; status:=final_status; return next;
end;
$$;
revoke all on function public.import_commute_records(uuid,uuid,text,text,jsonb,uuid,uuid) from public, anon;
grant execute on function public.import_commute_records(uuid,uuid,text,text,jsonb,uuid,uuid) to authenticated;
