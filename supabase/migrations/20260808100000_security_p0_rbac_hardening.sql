...
-- This migration closes a privilege-escalation path where broad organization managers
-- could assign roles more privileged than their own through invitation/member RPCs.
...
begin
  ...
  -- Owners may assign every supported organization role.
  ...
  -- Admins may manage operational roles but may not create owners or peer admins.
...
begin
  ...
  if invite_role not in ('owner','admin','program_admin','tdm_manager','sustainability_manager','site_manager','analyst','reviewer','participant') then
  ...
  if invite_site_id is not null and not exists (
    select 1 from public.organization_sites s
    where s.id = invite_site_id and s.organization_id = org_id
  ) then
    raise exception 'Site does not belong to organization';
  end if;
  if invite_cohort_id is not null and not exists (
    select 1 from public.cohorts c
    where c.id = invite_cohort_id and c.organization_id = org_id
  ) then
...
begin
  if uid is null then raise exception 'Authentication required'; end if;
  if not private.can_manage_members(org_id) then raise exception 'Organization member-management permission required'; end if;
  if new_role not in ('owner','admin','program_admin','tdm_manager','sustainability_manager','site_manager','analyst','reviewer','participant') then
  ...
  -- A caller may not create a role above the hierarchy they are permitted to assign.
  -- Existing owners are protected separately so an admin cannot demote/disable them.
  if current_role = 'owner' and not private.has_org_role(org_id, array['owner']) then
    raise exception 'Only an owner can modify another owner';
  end if;

  if not private.can_assign_role(org_id, new_role) then
    raise exception 'You may not assign the requested organization role';
  end if;
