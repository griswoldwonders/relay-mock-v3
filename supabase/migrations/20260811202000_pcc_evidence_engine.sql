...
-- Adds immutable baseline snapshots, observation periods, normalized commute observations,
-- metric values, findings, and RLS for organization-scoped evidence workflows.
...
create index if not exists commute_observations_period_idx on public.commute_observations(organization_id, site_id, baseline_id, observation_period_id);
create index if not exists commute_observations_participant_date_idx on public.commute_observations(organization_id, participant_key, observation_date);

create table if not exists public.evidence_validation_issues (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references public.organizations(id) on delete cascade,
  site_id uuid not null references public.organization_sites(id) on delete cascade,
  commute_observation_id uuid references public.commute_observations(id) on delete cascade,
  baseline_id uuid references public.evidence_baselines(id) on delete cascade,
  observation_period_id uuid references public.evidence_observation_periods(id) on delete cascade,
...
create policy commute_observations_manage on public.commute_observations for all to authenticated using (private.can_manage_org(organization_id)) with check (private.can_manage_org(organization_id));
...
create trigger commute_observations_set_updated_at before update on public.commute_observations for each row execute function public.set_updated_at();
...
create trigger audit_commute_observations after insert or update or delete on public.commute_observations for each row execute function private.write_audit_event();
